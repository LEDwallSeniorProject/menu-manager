...
class Maze(LEDWall.LEDProgram):
    ...
    def __bind_controls__(self):
        ...
        None
        """
        self.controller.add_function("UP",self.move_up)
        ...
        for row in range(len(self.matrix)):
            for col in range(len(self.matrix[row])):
                if self.matrix[row][col] == 1:	#because the maze has 4 pixels per unit, this draws cells with four add pixel statements
                    self.canvas.add(s.Pixel((2*col, 2*row), (100,100,100)))
                    self.canvas.add(s.Pixel((2*col-1, 2*row), (100,100,100)))
                    self.canvas.add(s.Pixel((2*col, 2*row-1), (100,100,100)))
                    self.canvas.add(s.Pixel((2*col-1, 2*row-1), (100,100,100)))
